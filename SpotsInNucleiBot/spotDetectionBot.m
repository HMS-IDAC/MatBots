classdef spotDetectionBot < handle    
    properties
        Prmts
        Figure
        Axis
    ...
    methods
        function bot = spotDetectionBot(S,nucleiChannel,nucleiMask)
            bot.NucMask = nucleiMask;
            ...
            bot.Prmts = zeros(nChannels,4); % channel, use, sigma, threshold
            bot.SDSigma = 2.0;
            bot.SDThr = 5.0;
            for i = 1:nChannels
            ...
            bot.ThrText = uicontrol('Parent',bot.Dialog,'Style','text','String',sprintf('threshold: %.02f', bot.SDThr),'HorizontalAlignment','left','Position',[2*dborder+(cwidth-dborder)/2 7*dborder+6*cheight (cwidth-dborder)/2 cheight]);
        ...
        function editThr(bot,src,callbckdata)
            ...
            elseif n <= 0
                ...
                bot.SDThr = n;
                bot.ThrText.String = sprintf('threshold: %.02f', bot.SDThr);
        ...
        function buttonDetectSpotsPushed(bot,src,callbackdata)
            bot.Spots = logPointSourceDetection(bot.Image, bot.NucMask, bot.SDSigma, bot.SDThr) > 0;
    ...
    methods (Static)
        function Mask = Headless(I, NucMask, sigma, sdThr)
            Mask = logPointSourceDetection(I, NucMask, sigma, sdThr) > 0;
        end
    end
end
